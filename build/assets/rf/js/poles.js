//if ( ! Detector.webgl ) Detector.addGetWebGLMessage();
			...
			function init() {
				...
				for ( i = 20; i < 25; i ++ ) {
                    ...
                    vertex3 = vertex1.clone();
                    vertex4 = vertex1.clone();
                
                /* length of lines */
                    
					vertex2.multiplyScalar( Math.random() * 1 + 1 + 1 + 1 + 1 + 1 );
			...
			function render() {
				...
				for ( var i = 0; i < scene.children.length; i ++ ) {
					...
					object.rotation.y = time * ( i < 100 ? ( i + 0.1 ) : - ( i + 0.1 ) );

						if ( i < 10 ) object.scale.x = object.scale.y = object.scale.z = object.originalScale * (i/1+1) * (1 + 0.05 * Math.sin( 1*time ) );
